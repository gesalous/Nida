...
// Function to connect to daemon
static int nvme_portails_connect_to_daemon(void)
{
    int ret;
    ...
    strncpy(addr.sun_path, "/tmp/nida.sock", sizeof(addr.sun_path) - 1);

    // Connect to daemon
    ret = kernel_connect(sock, (struct sockaddr *)&addr, sizeof(addr), 0);
    if (ret < 0) {
        pr_err("Failed to connect to daemon: %d\n", ret);
        sock_release(sock);
        return ret;
    }
...
// Function to send message to userspace daemon
static int nvme_portails_send_to_daemon(int type, const char *message)
{
    struct msghdr msg;
    struct kvec iov;
    struct sockaddr_un addr;
    struct nida_message nida_msg;
    int ret;

    memset(&nida_msg, 0, sizeof(nida_msg));
    nida_msg.type = type;
    strncpy(nida_msg.message, message, sizeof(nida_msg.message) - 1);

    memset(&addr, 0, sizeof(addr));
    addr.sun_family = AF_UNIX;
    strncpy(addr.sun_path, "/var/run/nida.sock", sizeof(addr.sun_path) - 1);

    memset(&msg, 0, sizeof(msg));
    msg.msg_name = &addr;
    msg.msg_namelen = sizeof(addr);

    iov.iov_base = &nida_msg;
    iov.iov_len = sizeof(nida_msg);
...
{
	nida_ib_dma_unmap_single(ibdev, qe->dma, capsule_size, dir);
	kfree(qe->data);
}
...
static void nvme_rdma_qp_event(struct ib_event *event, void *context)
{
	pr_debug("QP event %s (%d)\n",
		 nida_ib_event_msg(event->event), event->event);
...
static int nvme_rdma_create_qp(struct nvme_rdma_queue *queue, const int factor)
{
	...
	ret = nida_rdma_create_qp(queue->cm_id, dev->pd, &init_attr);
...
nvme_rdma_find_get_device(struct rdma_cm_id *cm_id)
{
	...
	ndev->pd = nida_ib_alloc_pd(ndev->dev,
...
static void nvme_rdma_free_cq(struct nvme_rdma_queue *queue)
{
	if (nvme_rdma_poll_queue(queue))
		nida_ib_free_cq(queue->ib_cq);
	else
		nida_ib_cq_pool_put(queue->ib_cq, queue->cq_size);
}
...
static void nvme_rdma_destroy_queue_ib(struct nvme_rdma_queue *queue)
{
	...
	if (queue->pi_support)
		nida_ib_mr_pool_destroy(queue->qp, &queue->qp->sig_mrs);
	nida_ib_mr_pool_destroy(queue->qp, &queue->qp->rdma_mrs);
		...
		struct nvme_rdma_queue *queue)
{
	...
	if (nvme_rdma_poll_queue(queue))
		queue->ib_cq = nida_ib_alloc_cq(ibdev, queue, queue->cq_size,
					   comp_vector, IB_POLL_DIRECT);
	else
		queue->ib_cq = nida_ib_cq_pool_get(ibdev, queue->cq_size,
...
static int nvme_rdma_create_queue_ib(struct nvme_rdma_queue *queue)
{
	...
	ret = nida_ib_mr_pool_init(queue->qp, &queue->qp->rdma_mrs,
			      queue->queue_size,
			      IB_MR_TYPE_MEM_REG,
			      pages_per_mr, 0);
	if (ret) {
	...
	if (queue->pi_support) {
		ret = nida_ib_mr_pool_init(queue->qp, &queue->qp->sig_mrs,
...
out_destroy_ring:
	nvme_rdma_free_ring(ibdev, queue->rsp_ring, queue->queue_size,
			    sizeof(struct nvme_completion), DMA_FROM_DEVICE);
out_destroy_qp:
	nida_rdma_destroy_qp(queue->cm_id);
out_destroy_ib_cq:
	nvme_rdma_free_cq(queue);
out_put_dev:
		...
		int idx, size_t queue_size)
{
	...
	ret = nida_rdma_resolve_addr(queue->cm_id, src_addr,
			(struct sockaddr *)&ctrl->addr,
			NVME_RDMA_CM_TIMEOUT_MS);
	if (ret) {
		dev_info(ctrl->ctrl.device,
			"nida_rdma_resolve_addr failed (%d).\n", ret);
...
out_destroy_cm_id:
	nida_rdma_destroy_id(queue->cm_id);
...
{
	nida_rdma_disconnect(queue->cm_id);
	nida_ib_drain_qp(queue->qp);
}
...
static void nvme_rdma_dma_unmap_req(struct ib_device *ibdev, struct request *rq)
{
	...
	if (blk_integrity_rq(rq)) {
		nida_ib_dma_unmap_sg(ibdev, req->metadata_sgl->sg_table.sgl,
				req->metadata_sgl->nents, rq_dma_dir(rq));
		sg_free_table_chained(&req->metadata_sgl->sg_table,
				      NVME_INLINE_METADATA_SG_CNT);
	}

	nida_ib_dma_unmap_sg(ibdev, req->data_sgl.sg_table.sgl, req->data_sgl.nents,
			rq_dma_dir(rq));
		...
		struct request *rq)
{
	...
	if (req->mr) {
		nida_ib_mr_pool_put(queue->qp, pool, req->mr);
		req->mr = NULL;
	}
		...
		int count)
{
	...
	req->mr = nida_ib_mr_pool_get(queue->qp, &queue->qp->rdma_mrs);
	...
	if (unlikely(nr < count)) {
		nida_ib_mr_pool_put(queue->qp, &queue->qp->rdma_mrs, req->mr);
		req->mr = NULL;
		if (nr < 0)
			return nr;
		return -EINVAL;
		...
		int count, int pi_count)
{
	...
	req->mr = nida_ib_mr_pool_get(queue->qp, &queue->qp->sig_mrs);
	if (WARN_ON_ONCE(!req->mr))
		return -EAGAIN;
...
mr_put:
	nida_ib_mr_pool_put(queue->qp, &queue->qp->sig_mrs, req->mr);
	req->mr = NULL;
	if (nr < 0)
		return nr;
	return -EINVAL;
		...
		int *count, int *pi_count)
{
	...
	*count = nida_ib_dma_map_sg(ibdev, req->data_sgl.sg_table.sgl,
	...
	if (blk_integrity_rq(rq)) {
		...
		*pi_count = nida_ib_dma_map_sg(ibdev,
					  req->metadata_sgl->sg_table.sgl,
...
static void nvme_rdma_submit_async_event(struct nvme_ctrl *arg)
{
	...
	nida_ib_dma_sync_single_for_device(dev, sqe->dma, sizeof(*cmd),
			DMA_TO_DEVICE);
...
static void nvme_rdma_recv_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	nida_ib_dma_sync_single_for_cpu(ibdev, qe->dma, len, DMA_FROM_DEVICE);
	/*
	...
	nida_ib_dma_sync_single_for_device(ibdev, qe->dma, len, DMA_FROM_DEVICE);
...
static int nvme_rdma_route_resolved(struct nvme_rdma_queue *queue)
{
	...
	ret = nida_rdma_connect_locked(queue->cm_id, &param);
	if (ret) {
		dev_err(ctrl->ctrl.device,
			"nida_rdma_connect_locked failed (%d).\n", ret);
		return ret;
	}
		...
		struct rdma_cm_event *ev)
{
	...
	nida_rdma_event_msg(ev->event), ev->event,
		ev->status, cm_id);
		...
		const struct blk_mq_queue_data *bd)
{
	...
	req->sqe.dma = nida_ib_dma_map_single(dev, req->sqe.data,
					 sizeof(struct nvme_command),
					 DMA_TO_DEVICE);
	err = nida_ib_dma_mapping_error(dev, req->sqe.dma);
	if (unlikely(err))
		return BLK_STS_RESOURCE;

	nida_ib_dma_sync_single_for_cpu(dev, sqe->dma,
			sizeof(struct nvme_command), DMA_TO_DEVICE);
	...
	nida_ib_dma_sync_single_for_device(dev, sqe->dma,
			sizeof(struct nvme_command), DMA_TO_DEVICE);
...
err:
	...
	nvme_cleanup_cmd(rq);
unmap_qe:
	nida_ib_dma_unmap_single(dev, req->sqe.dma, sizeof(struct nvme_command),
			    DMA_TO_DEVICE);
	return ret;
}
...
static void nvme_rdma_complete_rq(struct request *rq)
{
	...
	nvme_rdma_unmap_data(queue, rq);
	nida_ib_dma_unmap_single(ibdev, req->sqe.dma, sizeof(struct nvme_command),
			    DMA_TO_DEVICE);
	nvme_complete_rq(rq);
}
